...
Loads ``calibration/fit_summary.csv`` (emitted by the evaluation pipeline),
...
output table carries a raw ``<class>_score`` and a calibrated
``<class>_p_calibrated`` per class.

Classes whose calibration was skipped at training time (``status``
starting with anything other than ``fit``) get ``NaN`` in the
``_p_calibrated`` column so downstream consumers can detect "no
calibrated claim available".
"""
...
def apply_calibration_long(
    ...
    """Long ``[id, class, score]`` → adds ``p_calibrated``.

    Classes without a deployable calibrator (status not starting with
    ``fit``) get ``NaN`` in ``p_calibrated``. The classifier must be
    present in the fit summary; otherwise raises ``ValueError`` to make
    misconfiguration loud.
    """
    if long_df.empty:
        return long_df.assign(p_calibrated=pd.Series(dtype=float))
    ...
    return long_df.assign(p_calibrated=p_cal)
...
def assemble_output_table(
    ...
    *,
    ...
    """Pivot long ``[id, class, score, p_calibrated]`` → wide per-protein table.

    Output columns: ``id``, optionally ``sequence``, then for each class
    ``<class>_score`` and ``<class>_p_calibrated``.
    """
    ...
    p_wide = long_with_calibration.pivot(
        index="id", columns="class", values="p_calibrated",
    )
    p_wide.columns = [f"{c}_p_calibrated" for c in p_wide.columns]
    out = pd.concat([score_wide, p_wide], axis=1).reset_index()
