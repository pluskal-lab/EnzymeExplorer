...
def plot_role_axes_scatter(
    ...
    *,
    ...
    """One scatter panel per axis, all sharing the same y target. Useful for
    side-by-side visual comparison of which axis predicts the target.

    When ``weight_col`` is provided, rows with zero weight are dropped, the
    hexbin density is weighted by that column (so the colour scale reflects
    contribution to the regression), the regression line uses WLS, and the
    title quotes weighted Pearson r and ``n_eff``.
    """
    from scipy.stats import pearsonr  # type: ignore

    n_ax = len(axes)
    fig, axes_mpl = plt.subplots(1, n_ax, figsize=(5 * n_ax, 5), sharey=True)
    if n_ax == 1:
        axes_mpl = [axes_mpl]

    for ax, axis in zip(axes_mpl, axes):
        cols = [f"tm_{axis}", target_col] + ([weight_col] if weight_col else [])
        sub = pair_df[cols].dropna()
        if weight_col:
            sub = sub[sub[weight_col] > 0]
        if len(sub) < 5:
            ax.text(0.5, 0.5, f"only {len(sub)} valid pairs",
                    ha="center", transform=ax.transAxes)
            ax.set(title=axis)
            continue
        ...
        if weight_col:
            ...
            hb = ax.hexbin(x, y, C=w, reduce_C_function=np.sum,
                           gridsize=40, cmap="rocket_r", mincnt=1e-9, alpha=0.85)
            plt.colorbar(hb, ax=ax, label="Σ weight")
            sw = np.sqrt(w)
            Z = np.column_stack([np.ones_like(x), x])
            beta, *_ = np.linalg.lstsq(Z * sw[:, None], y * sw, rcond=None)
            intercept, slope = float(beta[0]), float(beta[1])
            from enzymeexplorer.src.domain_clustering import catalytic_role as _cr  # type: ignore
            r, p, n_eff = _cr._weighted_pearson(x, y, w)
            n_label = f"n={len(sub)} (n_eff={n_eff:.0f})"
        else:
            hb = ax.hexbin(x, y, gridsize=40, cmap="rocket_r", mincnt=1, alpha=0.85)
            plt.colorbar(hb, ax=ax, label="# pairs")
            slope, intercept = np.polyfit(x, y, 1)
            r, p = pearsonr(x, y)
            n_label = f"n={len(sub)}"
        xs = np.linspace(0, 1, 100)
        ax.plot(xs, slope * xs + intercept, color="white", linewidth=2.2, alpha=0.9)
        ax.plot(xs, slope * xs + intercept, color="black", linewidth=1)
        weighted_tag = "weighted " if weight_col else ""
        ax.set(
            xlabel=f"{axis}-{axis} TM",
            xlim=(0, 1.02), ylim=(-0.02, 1.05),
            title=(f"{axis}    {weighted_tag}Pearson r = {r:.3f}"
                   f"  (p = {p:.2e})\n  slope = {slope:.3f}   {n_label}"),
        )
        sns.despine(ax=ax)
    axes_mpl[0].set_ylabel(target_label)
    fig.suptitle(
        f"axis vs {target_label}  {title_suffix}".strip(),
        fontsize=12,
    )
    fig.tight_layout()
    fig.savefig(output_path, dpi=150, bbox_inches="tight")
...
def plot_partial_corr_per_axis(
    corr_df: pd.DataFrame,
    *,
    axes: tuple[str, ...],
    output_path: str | Path,
    title: str,
    target_order: list[tuple[str, str]] | None = None,
) -> None:
    """Grouped bar chart of marginal vs partial Pearson r per axis.

    One panel per similarity target. Every axis contributes a pair of
    bars (marginal | partial). Hypothesis supported when one axis's
    partial r stays high while the others' drop toward zero.
    """
    if target_order is None:
        target_order = [
            ("reaction Jaccard", "reaction-tag Jaccard"),
            ("substrate MCS Tanimoto", "substrate MCS Tan."),
            ("product avg-Tanimoto (per shared substrate, then mean)",
             "product avg-Tan. (per shared substrate)"),
        ]
    n_panels = len(target_order)
    fig, axes_mpl = plt.subplots(
        1, n_panels, figsize=(4.4 * n_panels, 5.5), sharey=True,
    )
    if n_panels == 1:
        axes_mpl = [axes_mpl]
    bar_w = 0.35
    x = np.arange(len(axes))
    palette = sns.color_palette("Set2", n_colors=len(axes))

    for ax_mpl, (target_key, target_label) in zip(axes_mpl, target_order):
        rows = corr_df[corr_df["target"] == target_key].set_index("axis")
        marg = [float(rows.loc[a, "marginal_r"]) if a in rows.index else float("nan") for a in axes]
        part = [float(rows.loc[a, "partial_r"]) if a in rows.index else float("nan") for a in axes]

        for i, axis in enumerate(axes):
            ax_mpl.bar(i - bar_w / 2, marg[i], bar_w,
                       color=palette[i], alpha=0.5,
                       edgecolor="black", linewidth=0.4,
                       label="marginal" if i == 0 else None)
            ax_mpl.bar(i + bar_w / 2, part[i], bar_w,
                       color=palette[i], edgecolor="black", linewidth=0.6,
                       label="partial" if i == 0 else None)
            for off, v in ((-bar_w / 2, marg[i]), (bar_w / 2, part[i])):
                if pd.isna(v):
                    continue
                y = v + 0.012 if v >= 0 else v - 0.018
                ax_mpl.text(i + off, y, f"{v:.2f}",
                            ha="center", va="bottom" if v >= 0 else "top",
                            fontsize=7)
        ax_mpl.axhline(0, color="black", linewidth=0.6)
        ax_mpl.set_xticks(x)
        ax_mpl.set_xticklabels(list(axes), fontsize=10)
        ax_mpl.set(title=target_label, xlabel="axis (domain type)")
        ax_mpl.legend(fontsize=8, loc="upper right")
        sns.despine(ax=ax_mpl)
    axes_mpl[0].set_ylabel("Pearson r")
    # Y-limits chosen for visual comparability.
    all_vals = corr_df[["marginal_r", "partial_r"]].values.flatten()
    all_vals = all_vals[~pd.isna(all_vals)]
    if all_vals.size:
        lo = min(all_vals.min(), -0.2) - 0.05
        hi = max(all_vals.max(), 0.5) + 0.08
        for ax_mpl in axes_mpl:
            ax_mpl.set_ylim(lo, hi)
...
def plot_partial_corr_overview(
    correlations: dict[str, pd.DataFrame],
    axes_per_config: dict[str, tuple[str, ...]],
    output_path: str | Path,
    target_order: list[tuple[str, str]] | None = None,
) -> None:
    """Cross-(hypothesis, config) panel summary.

    Layout: N_targets columns × N_configs rows. Each cell: bar chart of
    partial Pearson r per axis. Lets the user see at a glance, across
    all hypotheses + configs, where the partial-r signal concentrates.
    """
    if target_order is None:
        target_order = [
            ("reaction Jaccard", "reaction-tag Jaccard"),
    ...
    for row, cfg_tag in enumerate(cfgs):
        corr = correlations[cfg_tag]
        axes_for_cfg = axes_per_config.get(cfg_tag, ())
        for col, (target_key, target_label) in enumerate(target_order):
            ax = axes_grid[row, col]
            rows_t = corr[corr["target"] == target_key].set_index("axis")
            marg = [
                float(rows_t.loc[a, "marginal_r"]) if a in rows_t.index else float("nan")
                for a in axes_for_cfg
            ]
            part = [
                float(rows_t.loc[a, "partial_r"]) if a in rows_t.index else float("nan")
                for a in axes_for_cfg
            ]
            n_ax = len(axes_for_cfg)
            xs = np.arange(n_ax)
            bar_w = 0.35
            for i, axis in enumerate(axes_for_cfg):
                ax.bar(i - bar_w / 2, marg[i], bar_w,
                       color=palette[i % len(palette)], alpha=0.5,
                       edgecolor="black", linewidth=0.4)
                ax.bar(i + bar_w / 2, part[i], bar_w,
                       color=palette[i % len(palette)],
                       edgecolor="black", linewidth=0.6)
                for off, v in ((-bar_w / 2, marg[i]), (bar_w / 2, part[i])):
                    if pd.isna(v):
                        continue
                    y = v + 0.015 if v >= 0 else v - 0.022
                    ax.text(i + off, y, f"{v:.2f}",
                            ha="center", va="bottom" if v >= 0 else "top",
                            fontsize=6)
            ax.axhline(0, color="black", linewidth=0.5)
            ax.set_xticks(xs)
            ax.set_xticklabels(list(axes_for_cfg), fontsize=9)
            if col == 0:
                ax.set_ylabel(cfg_tag, fontsize=9)
            if row == 0:
                ax.set_title(target_label, fontsize=10)
            sns.despine(ax=ax)

    # Y-limits across all panels.
    all_vals = []
    for corr in correlations.values():
        all_vals.extend(corr[["marginal_r", "partial_r"]].values.flatten())
    all_vals = [v for v in all_vals if not pd.isna(v)]
    if all_vals:
        lo = min(min(all_vals), -0.2) - 0.05
        hi = max(max(all_vals), 0.5) + 0.08
        for row in range(n_rows):
            for col in range(n_cols):
                axes_grid[row, col].set_ylim(lo, hi)

    # Custom global legend for marginal vs partial.
    legend_handles = [
        plt.Rectangle((0, 0), 1, 1, color="grey", alpha=0.5),
        plt.Rectangle((0, 0), 1, 1, color="grey"),
    ]
    fig.legend(
        legend_handles, ["marginal r", "partial r (control other axes)"],
        loc="upper center", ncol=2, fontsize=10, bbox_to_anchor=(0.5, 1.02),
    )
    fig.suptitle(
        "Partial-correlation overview — does each domain axis predict the target?",
        y=1.06, fontsize=12,
    )
    fig.tight_layout()
    fig.savefig(output_path, dpi=150, bbox_inches="tight")
...
def plot_variance_partition(
    partitions_per_target: dict[str, dict],
    axes: tuple[str, ...],
    output_path: str | Path,
    *,
    title: str = "",
) -> None:
    """Stacked-bar plot of variance partition per target.

    ``partitions_per_target``: ``{target_label: variance_partition_dict}``
    where each dict has keys ``unique_<axis>``, ``shared``, ``unexplained``,
    ``r2_full``, ``n``.

    For each target, render a single stacked bar on a 0-1 axis. Components
    in this order from bottom: unique-α, unique-β, (unique-γ), shared
    (with hatch when negative), unexplained.
    """
    targets = list(partitions_per_target.keys())
    if not targets:
        return

    axis_palette = sns.color_palette("Set2", n_colors=max(len(axes), 3))
    shared_color = "#dcdcdc"
    unexplained_color = "#f5f5f5"

    fig, ax = plt.subplots(figsize=(max(7, 2.5 * len(targets)), 5))
    ...
    for ai, axis in enumerate(axes):
        vals = np.array(
            [partitions_per_target[t].get(f"unique_{axis}", 0.0) for t in targets]
        )
        ax.bar(x, vals, bar_w, bottom=bottoms,
               label=f"unique to {axis}",
               color=axis_palette[ai], edgecolor="white", linewidth=0.5)
        for xi, v in enumerate(vals):
            if v >= 0.01:
                ax.text(xi, bottoms[xi] + v / 2, f"{v:.3f}",
                        ha="center", va="center", fontsize=7,
                        color="black", fontweight="bold")
        bottoms += vals

    shared_vals = np.array(
        [partitions_per_target[t].get("shared", 0.0) for t in targets]
    )
    pos_shared = np.where(shared_vals > 0, shared_vals, 0.0)
    neg_shared = np.where(shared_vals < 0, shared_vals, 0.0)
    if pos_shared.any():
        ax.bar(x, pos_shared, bar_w, bottom=bottoms,
               color=shared_color, edgecolor="black", linewidth=0.5,
               label="shared (colinearity)")
        for xi, v in enumerate(pos_shared):
            if v >= 0.01:
                ax.text(xi, bottoms[xi] + v / 2, f"{v:.3f}",
                        ha="center", va="center", fontsize=7, color="black")
        bottoms_pos = bottoms + pos_shared
    else:
        bottoms_pos = bottoms.copy()
    if neg_shared.any():
        # Place the negative bar going down from y=0 — visually it's a "gap"
        # showing suppression. Keep it as a separate aesthetics block.
        for xi, v in enumerate(neg_shared):
            if v < -0.005:
                ax.bar(xi, v, bar_w, bottom=0,
                       color="#ffd6d6", edgecolor="red", hatch="//",
                       linewidth=0.5,
                       label="suppressor (shared < 0)" if xi == 0 else None)
                ax.text(xi, v / 2, f"{v:+.3f}",
                        ha="center", va="center", fontsize=7, color="red")

    unexplained = np.array(
        [partitions_per_target[t].get("unexplained", 0.0) for t in targets]
    )
    ax.bar(x, unexplained, bar_w, bottom=bottoms_pos,
           color=unexplained_color, edgecolor="grey", linewidth=0.4,
           label="unexplained")

    ax.set_xticks(x)
    label_targets = [
        f"{t}\n(R²={partitions_per_target[t].get('r2_full', 0):.3f}, n={int(partitions_per_target[t].get('n', 0))})"
        for t in targets
    ]
    ax.set_xticklabels(label_targets, fontsize=9)
    ax.set(
        ylabel="fraction of target variance",
        ylim=(min(neg_shared.min() if neg_shared.size else 0, -0.02), 1.05),
        title=f"Variance partition  —  {title}".strip(" —"),
    )
    ax.axhline(0, color="black", linewidth=0.5)
...
def plot_target_target_matrix(
    ...
    *,
    ...
    """Pair-plot-style 3×3 matrix of target columns.

    Diagonal: KDE of each target's distribution.
    Off-diagonal: hexbin density + Pearson r in title.

    When ``weights_per_target`` is provided (mapping target_col → weight
    column name), the diagonal histograms are weighted by that target's
    weight and the off-diagonal cells are weighted by the geometric mean
    ``sqrt(w_i · w_j)`` (rows where either weight is 0 are dropped). The
    weighted Pearson r is reported in the cell title.
    """
    from scipy.stats import pearsonr  # type: ignore
    ...
    if n == 0:
        return
    fig, axes_grid = plt.subplots(n, n, figsize=(4.2 * n, 3.8 * n))
    if n == 1:
        axes_grid = np.array([[axes_grid]])

    weighted = bool(weights_per_target)

    for i, (col_i, lbl_i) in enumerate(targets):
        for j, (col_j, lbl_j) in enumerate(targets):
            ax = axes_grid[i, j]
            if i == j:
                wcol = (weights_per_target or {}).get(col_i)
                if weighted and wcol and wcol in pair_df.columns:
                    sub = pair_df[[col_i, wcol]].dropna()
                    sub = sub[sub[wcol] > 0]
                    if len(sub) >= 2 and sub[col_i].std() > 0:
                        sns.histplot(
                            x=sub[col_i].values, weights=sub[wcol].values,
                            bins=30, ax=ax, color="steelblue",
                            stat="density", kde=True, edgecolor="white",
                        )
                else:
                    vals = pair_df[col_i].dropna()
                    if len(vals) >= 2 and vals.std() > 0:
                        sns.histplot(vals, bins=30, ax=ax, color="steelblue",
                                     stat="density", kde=True, edgecolor="white")
                ax.set(title=lbl_i, xlabel="", ylabel="density")
            else:
                wcol_i = (weights_per_target or {}).get(col_i)
                wcol_j = (weights_per_target or {}).get(col_j)
                if (weighted and wcol_i and wcol_j
                        and wcol_i in pair_df.columns and wcol_j in pair_df.columns):
                    sub = pair_df[[col_i, col_j, wcol_i, wcol_j]].dropna()
                    sub = sub[(sub[wcol_i] > 0) & (sub[wcol_j] > 0)]
                    w = np.sqrt(sub[wcol_i].values * sub[wcol_j].values)
                else:
                    sub = pair_df[[col_i, col_j]].dropna()
                    w = None
                if len(sub) >= 5:
                    if w is not None:
                        hb = ax.hexbin(
                            sub[col_j].values, sub[col_i].values,
                            C=w, reduce_C_function=np.sum,
                            gridsize=30, cmap="rocket_r", mincnt=1e-9, alpha=0.85,
                        )
                        plt.colorbar(hb, ax=ax, label="Σ weight")
                        from enzymeexplorer.src.domain_clustering import catalytic_role as _cr  # type: ignore
                        r, _, n_eff = _cr._weighted_pearson(
                            sub[col_j].values.astype(float),
                            sub[col_i].values.astype(float), w,
                        )
                        ax.set_title(
                            f"weighted r = {r:.3f}   n_eff={n_eff:.0f}",
                            fontsize=9,
                        )
                    else:
                        hb = ax.hexbin(
                            sub[col_j], sub[col_i],
                            gridsize=30, cmap="rocket_r", mincnt=1, alpha=0.85,
                        )
                        plt.colorbar(hb, ax=ax, label="# pairs")
                        r, _ = pearsonr(sub[col_j].values, sub[col_i].values)
                        ax.set_title(f"r = {r:.3f}", fontsize=9)
                if j == 0:
                    ax.set_ylabel(lbl_i, fontsize=9)
                else:
                    ax.set_ylabel("")
                if i == n - 1:
                    ax.set_xlabel(lbl_j, fontsize=9)
                else:
                    ax.set_xlabel("")
            sns.despine(ax=ax)
    suffix = " (weighted)" if weighted else ""
    fig.suptitle(
        f"Target × target relationships{suffix}  —  {title}".strip(" —"),
        fontsize=11,
    )
    fig.tight_layout()
    fig.savefig(output_path, dpi=150, bbox_inches="tight")
...
def plot_feature_correlation_heatmap(
    ...
    *,
    title: str = "",
    weights_per_target: dict[str, str] | None = None,
    default_weight_col: str | None = None,
) -> None:
    """Pearson-r heatmap over (axis TMs ∪ target columns).

    When weights are supplied, each cell uses pairwise-complete weighted
    Pearson r:

      * axis vs axis:           ``default_weight_col`` if given (e.g.
                                ``w_cluster``) — the redundancy weight that
                                applies to all pairs.
      * axis vs target:         that target's weight from ``weights_per_target``.
      * target_i vs target_j:   geometric mean ``sqrt(w_i · w_j)``.
    """
    from enzymeexplorer.src.domain_clustering import catalytic_role as _cr  # type: ignore

    cols = [(f"tm_{ax}", f"tm_{ax}") for ax in axes] + targets
    col_keys = [c for c, _ in cols if c in pair_df.columns]
    col_labels = [lbl for c, lbl in cols if c in pair_df.columns]
    if len(col_keys) < 2:
        return
    target_cols = {c for c, _ in targets}
    weighted = bool(weights_per_target) or bool(default_weight_col)
    weights_per_target = weights_per_target or {}

    def _weight_for(c_i: str, c_j: str) -> np.ndarray | None:
        if not weighted:
            return None
        if c_i in target_cols and c_j in target_cols:
            wi = weights_per_target.get(c_i)
            wj = weights_per_target.get(c_j)
            if wi and wj and wi in pair_df.columns and wj in pair_df.columns:
                return np.sqrt(
                    pair_df[wi].fillna(0).values * pair_df[wj].fillna(0).values
                )
            return None
        if c_i in target_cols:
            wi = weights_per_target.get(c_i)
            if wi and wi in pair_df.columns:
                return pair_df[wi].fillna(0).values
        if c_j in target_cols:
            wj = weights_per_target.get(c_j)
            if wj and wj in pair_df.columns:
                return pair_df[wj].fillna(0).values
        if default_weight_col and default_weight_col in pair_df.columns:
            return pair_df[default_weight_col].fillna(0).values
        return None

    n_cols = len(col_keys)
    M = np.full((n_cols, n_cols), np.nan)
    n_per_cell = np.zeros((n_cols, n_cols), dtype=int)
    for i, ci in enumerate(col_keys):
        for j, cj in enumerate(col_keys):
            if i == j:
                M[i, j] = 1.0
                n_per_cell[i, j] = pair_df[ci].notna().sum()
                continue
            xi = pair_df[ci].values.astype(float)
            xj = pair_df[cj].values.astype(float)
            ...
            if w is not None:
                ...
                if mask.sum() < 5:
                    continue
                r, _, _ = _cr._weighted_pearson(xi[mask], xj[mask], w[mask])
                M[i, j] = r
            else:
                if mask.sum() < 5:
                    continue
                M[i, j] = float(np.corrcoef(xi[mask], xj[mask])[0, 1])
            n_per_cell[i, j] = int(mask.sum())

    fig, ax = plt.subplots(figsize=(0.95 * n_cols + 3, 0.95 * n_cols + 2))
    sns.heatmap(
        M, ax=ax, annot=True, fmt=".2f",
        cmap="RdBu_r", vmin=-1, vmax=1, center=0,
        xticklabels=col_labels, yticklabels=col_labels,
        cbar_kws={"label": ("weighted Pearson r" if weighted else "Pearson r")},
        linewidths=0.4, linecolor="white",
    )
    off_diag = ~np.eye(n_cols, dtype=bool)
    n_min = int(n_per_cell[off_diag].min()) if n_cols > 1 else 0
    n_max = int(n_per_cell[off_diag].max()) if n_cols > 1 else 0
    suffix = " (weighted)" if weighted else ""
    ax.set(title=(
        f"Feature correlation heatmap{suffix}  —  {title}".strip(" —")
        + f"\n(pairwise-complete obs.; per-cell n ∈ [{n_min}, {n_max}])"
    ))
...
def plot_stratified_correlation(
    ...
    *,
    ...
    """Plot Pearson r of (axis TM, target) within bins of substrate-MCS.

    ``stratified_df`` is the output of
    :func:`catalytic_role.stratified_correlations` — long-form table with
    columns ``axis``, ``bin_label``, ``n``, ``pearson_r``.
    """
    if stratified_df.empty:
        return
    fig, ax = plt.subplots(figsize=(11, 5))

    bin_order = list(dict.fromkeys(stratified_df["bin_label"].tolist()))
    n_bins = len(bin_order)
    bar_w = 0.8 / max(len(axes), 1)
    palette = sns.color_palette("Set2", n_colors=len(axes))

    for ai, axis in enumerate(axes):
        sub = stratified_df[stratified_df["axis"] == axis].set_index("bin_label").reindex(bin_order)
        offsets = np.arange(n_bins) + (ai - (len(axes) - 1) / 2) * bar_w
        ax.bar(offsets, sub["pearson_r"].fillna(0).values, bar_w,
               color=palette[ai], edgecolor="black", linewidth=0.5,
               label=axis)
        for xi, v, n in zip(offsets, sub["pearson_r"].values, sub["n"].values):
            if pd.isna(v):
                continue
            y = v + 0.015 if v >= 0 else v - 0.025
            ax.text(xi, y, f"{v:+.2f}",
                    ha="center", va="bottom" if v >= 0 else "top", fontsize=7)

    n_pairs_label = [
        f"{b}\nn={int(stratified_df[stratified_df['bin_label'] == b]['n'].iloc[0]) if (stratified_df['bin_label'] == b).any() else 0}"
        for b in bin_order
    ]
    ax.axhline(0, color="black", linewidth=0.5)
    ax.set_xticks(np.arange(n_bins))
    ax.set_xticklabels(n_pairs_label, fontsize=8)
    ax.set(
        xlabel="substrate-MCS bin (with within-bin n)",
        ylabel=f"Pearson r vs {target_label}",
        title=f"Stratified correlation  —  {title}".strip(" —"),
    )
...
def plot_weighted_vs_unweighted_comparison(
    corr_unweighted: pd.DataFrame,
    corr_weighted: pd.DataFrame,
    partitions_unweighted: dict[str, dict],
    partitions_weighted: dict[str, dict],
    axes: tuple[str, ...],
    output_path: str | Path,
    *,
    title: str = "",
) -> None:
    """Side-by-side weighted-vs-unweighted comparison for one (hypothesis, config).

    Two-row figure:
      Row 1: grouped bars of partial Pearson r per (axis, target).
             Each (axis, target) contributes two bars — unweighted vs weighted.
      Row 2: grouped bars of variance-partition unique R² per (axis, target).

    Bars rendered in muted grey for unweighted, saturated colour for
    weighted. ``n_eff`` shown above weighted bars when it differs from the
    raw row count (i.e. the weighting actually shrank the effective sample).
    """
    targets = list(partitions_unweighted.keys()) or list(partitions_weighted.keys())
    n_axes = len(axes)
    n_targets = len(targets)
    if n_axes == 0 or n_targets == 0:
        return

    fig, ax_grid = plt.subplots(2, 1, figsize=(max(8, 2.6 * n_axes * n_targets), 8))
    palette = sns.color_palette("Set2", n_colors=n_axes)
    grey = "#bdbdbd"

    cluster_centres = np.arange(n_axes * n_targets)
    bar_w = 0.38

    def _lookup_partial(corr_df: pd.DataFrame, axis: str, target_label: str) -> float:
        if corr_df is None or corr_df.empty:
            return float("nan")
        m = (corr_df["axis"] == axis) & (corr_df["target"] == target_label)
        if not m.any():
            return float("nan")
        return float(corr_df.loc[m, "partial_r"].iloc[0])

    def _lookup_neff(corr_df: pd.DataFrame, axis: str, target_label: str) -> float:
        if corr_df is None or corr_df.empty or "n_eff" not in corr_df.columns:
            return float("nan")
        m = (corr_df["axis"] == axis) & (corr_df["target"] == target_label)
        if not m.any():
            return float("nan")
        return float(corr_df.loc[m, "n_eff"].iloc[0])

    # ---- Row 1: partial Pearson r ----
    ax = ax_grid[0]
    xticks_lbl: list[str] = []
    centres = []
    for ti, target_label in enumerate(targets):
        for ai, axis_name in enumerate(axes):
            ci = ti * n_axes + ai
            centres.append(ci)
            xticks_lbl.append(f"{axis_name}\n{target_label}")
            r_u = _lookup_partial(corr_unweighted, axis_name, target_label)
            r_w = _lookup_partial(corr_weighted, axis_name, target_label)
            ax.bar(ci - bar_w / 2, r_u if not pd.isna(r_u) else 0, bar_w,
                   color=grey, edgecolor="black", linewidth=0.5,
                   label="unweighted" if ci == 0 else None)
            ax.bar(ci + bar_w / 2, r_w if not pd.isna(r_w) else 0, bar_w,
                   color=palette[ai], edgecolor="black", linewidth=0.5,
                   label="weighted" if ci == 0 else None)
            if not pd.isna(r_u):
                ax.text(ci - bar_w / 2, r_u + (0.01 if r_u >= 0 else -0.025),
                        f"{r_u:+.2f}", ha="center", fontsize=7,
                        va="bottom" if r_u >= 0 else "top")
            if not pd.isna(r_w):
                ax.text(ci + bar_w / 2, r_w + (0.01 if r_w >= 0 else -0.025),
                        f"{r_w:+.2f}", ha="center", fontsize=7,
                        va="bottom" if r_w >= 0 else "top", fontweight="bold")
        if ti < n_targets - 1:
            ax.axvline(ti * n_axes + n_axes - 0.5, color="lightgrey", linewidth=0.6)

    ax.axhline(0, color="black", linewidth=0.5)
    ax.set_xticks(centres)
    ax.set_xticklabels(xticks_lbl, fontsize=7)
    ax.set_ylabel("partial Pearson r")
    ax.set_title("Partial r — unweighted (grey) vs weighted (coloured)")
    ...
    xticks_lbl = []
    for ti, target_label in enumerate(targets):
        part_u = partitions_unweighted.get(target_label, {}) if partitions_unweighted else {}
        part_w = partitions_weighted.get(target_label, {}) if partitions_weighted else {}
        for ai, axis_name in enumerate(axes):
            ci = ti * n_axes + ai
            centres.append(ci)
            xticks_lbl.append(f"{axis_name}\n{target_label}")
            u_unw = float(part_u.get(f"unique_{axis_name}", float("nan")))
            u_wgt = float(part_w.get(f"unique_{axis_name}", float("nan")))
            ax.bar(ci - bar_w / 2, u_unw if not pd.isna(u_unw) else 0, bar_w,
                   color=grey, edgecolor="black", linewidth=0.5)
            ax.bar(ci + bar_w / 2, u_wgt if not pd.isna(u_wgt) else 0, bar_w,
                   color=palette[ai], edgecolor="black", linewidth=0.5)
            if not pd.isna(u_unw):
                ax.text(ci - bar_w / 2, u_unw + 0.002,
                        f"{u_unw:.3f}", ha="center", fontsize=7, va="bottom")
            if not pd.isna(u_wgt):
                ax.text(ci + bar_w / 2, u_wgt + 0.002,
                        f"{u_wgt:.3f}", ha="center", fontsize=7,
                        va="bottom", fontweight="bold")
        if ti < n_targets - 1:
            ax.axvline(ti * n_axes + n_axes - 0.5, color="lightgrey", linewidth=0.6)

        # n / n_eff annotation under the cluster of bars for this target.
        n_u = part_u.get("n", float("nan"))
        n_w = part_w.get("n", float("nan"))
        ne_u = part_u.get("n_eff", n_u)
        ne_w = part_w.get("n_eff", n_w)
        ax.text((ti * n_axes + n_axes / 2) - 0.5, -0.005,
                f"unw n={int(n_u) if not pd.isna(n_u) else '?'}"
                f"   wgt n={int(n_w) if not pd.isna(n_w) else '?'}"
                f" (n_eff={ne_w:.0f})",
    ...
    ax.set_xticks(centres)
    ax.set_xticklabels(xticks_lbl, fontsize=7)
    ax.set_ylabel("unique R² (variance partition)")
    ax.set_title("Unique R² — unweighted (grey) vs weighted (coloured)")
    sns.despine(ax=ax)

    fig.suptitle(f"Weighted vs unweighted  —  {title}".strip(" —"), fontsize=11)
    fig.tight_layout()
    fig.savefig(output_path, dpi=150, bbox_inches="tight")
    plt.close(fig)
    logger.info("Saved %s", output_path)
...
def plot_target_distributions_overview(
    pair_dfs_per_config: dict[str, pd.DataFrame],
    targets: list[tuple[str, str]],
    output_path: str | Path,
    *,
    weights_per_target: dict[str, str] | None = None,
) -> None:
    """Histogram-grid: rows = configs, columns = targets.

    Mirrors the (config, target) layout of partial_correlations_overview
    so they can be eyeballed side-by-side. When ``weights_per_target`` is
    supplied, each histogram is weighted by that target's weight column so
    the displayed distribution reflects the contribution to the regression.
    """
    cfgs = list(pair_dfs_per_config.keys())
    n_rows = len(cfgs)
    n_cols = len(targets)
    ...
    for r, cfg in enumerate(cfgs):
        df = pair_dfs_per_config[cfg]
        for c, (col, lbl) in enumerate(targets):
            ax = axes_grid[r, c]
            wcol = (weights_per_target or {}).get(col)
            if weighted and wcol and wcol in df.columns:
                sub = df[[col, wcol]].dropna()
                sub = sub[sub[wcol] > 0]
                if len(sub) >= 2 and sub[col].std() > 0:
                    w = sub[wcol].values.astype(float)
                    n_eff = float(w.sum() ** 2 / (w * w).sum()) if (w * w).sum() > 0 else 0
                    sns.histplot(
                        x=sub[col].values, weights=w,
                        bins=30, ax=ax,
                        color=palette[c % len(palette)],
                        stat="density", kde=True, edgecolor="white",
                    )
                    ax.set(title=(
                        f"{lbl}\nn={len(sub)} (n_eff={n_eff:.0f})  "
                        f"weighted mean={float((w*sub[col]).sum()/w.sum()):.2f}"
                    ))
                else:
            ...
            sns.despine(ax=ax)
    suffix = " (weighted)" if weighted else ""
    fig.suptitle(f"Target distributions per (config × target){suffix}",
                 fontsize=12, y=1.01)
    fig.tight_layout()
    fig.savefig(output_path, dpi=150, bbox_inches="tight")
    plt.close(fig)
